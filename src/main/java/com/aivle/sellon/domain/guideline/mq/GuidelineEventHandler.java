package com.aivle.sellon.domain.guideline.mq;
...
public class GuidelineEventHandler implements MqEventHandler {

    private static final String GUIDELINE_GENERATED_EVENT_TYPE = "ai.guideline.generated";
    ...
    public boolean supports(String eventType) {
        return GUIDELINE_GENERATED_EVENT_TYPE.equals(eventType);
    }
    ...
    public void handle(JsonNode envelope, String eventId, String traceId) {
        ...
        try {
            guidelineService.saveGeneratedGuideline(companyKey, payload);
        } catch (CompanyNotFoundException e) {
            throw deadLetter(EventFailureReason.UNKNOWN_COMPANY, eventId, traceId, e);
        }
    }

    private GuidelinePayload readPayload(JsonNode envelope) {
        JsonNode payloadNode = envelope.get("payload");
        if (payloadNode == null || payloadNode.isNull())
            throw new IllegalArgumentException("payload 없음");

        GuidelinePayload payload = jsonMapper.treeToValue(payloadNode, GuidelinePayload.class);
        validate(payload);
        return payload;
    }
     ...
     */
    private void validate(GuidelinePayload payload) {
        if (payload.guidelineId() == null || payload.guidelineId().isBlank())
            throw new IllegalArgumentException("guideline_id 없음");
        ...
        if (payload.sourcePayload() == null || payload.sourcePayload().isNull())
            throw new IllegalArgumentException("source_payload 없음");
    }
    ...
    private AmqpRejectAndDontRequeueException deadLetter(
        ...
        log.error("이벤트 DLQ 이동 - reason={}, detail={}, eventType={}, eventId={}, traceId={}",
                reason, reason.getDescription(), GUIDELINE_GENERATED_EVENT_TYPE, eventId, traceId, cause);
